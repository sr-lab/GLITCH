...
attr_has_any_checksum(attr, checksum_values) if {
...
check_integrity_check_atomic_unit(node) if {
...
check_integrity_check_keyvalues(node) if {
	value = data.security.checksum[_]
	glitch_lib.has_substring(node.name, value)
