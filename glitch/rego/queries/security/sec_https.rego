...
check_http_without_tls(node) if {
	node.ir_type == "Sum"
...
url_is_http(url) if {
	...
	not glitch_lib.has_substring(url, "https://")
} else if {
...
url_has_http_or_www(url) if {
	glitch_lib.has_substring(url, "http://")
} else if {
	glitch_lib.has_substring(url, "www")
}

url_is_whitelisted(url) if {
	host := extract_hostname(url)
	host == data.security.url_http_white_list[_]
}

extract_hostname(value) := output if {
	parts := split(value, "://")
...
select_rest(parts, original) := output if {
	count(parts) > 1
	output := parts[1]
} else := output if {
