...
def element_from_dict(data: Dict[str, Any]) -> CodeElement:
    ...
    if "key" in data and "value" in data and "ir_type" not in data:
        ...
        assert isinstance(
            value_elem, Expr
        ), f"Value in Key Value Hash mapping {value_elem.__class__.__name__} is not an Expr"
    ...
    if ir_type == "String":
        ...
        for v in values_data:
            ...
            assert isinstance(
                elem, Expr
            ), f"Element in Array {elem.__class__.__name__} is not an Expr"
            values.append(elem)
        ...
        for pair in data["value"]:
            ...
            assert isinstance(
                key, Expr
            ), f"Key in Hash Mapping {key.__class__.__name__} is not an Expr"
            assert isinstance(
                val, Expr
            ), f"Value in Hash Mapping {val.__class__.__name__} is not an Expr"
            hash_value[key] = val
        ...
        assert isinstance(
            value_elem, Expr
        ), f"value in Attribute {value_elem.__class__.__name__} is not an Expr"
        ...
        assert isinstance(
            value_elem, Expr
        ), f"value in Variable {value_elem.__class__.__name__} is not an Expr"
        ...
        assert isinstance(
            value_elem, Expr
        ), f"value in KeyValue {value_elem.__class__.__name__} is not an Expr"
    ...
    # Expressions and binary/unary operations
        ...
        assert isinstance(
            left_elem, Expr
        ), f"left_elem in BinaryOperation {ir_type}, {left_elem.__class__.__name__} is not an Expr"
        assert isinstance(
            right_elem, Expr
        ), f"right_elem in BinaryOperation {ir_type}, {right_elem.__class__.__name__} is not an Expr"
        ...
        assert isinstance(
            expr_elem, Expr
        ), f"expr_elem in UnaryRelation {ir_type}, {expr_elem.__class__.__name__} is not an Expr"
        ...
        for a in args_data:
            ...
            assert isinstance(
                arg_elem, Expr
            ), f"arg_elem in FunctionCall {arg_elem.__class__.__name__} is not an Expr"
        ...
        assert isinstance(
            receiver_elem, Expr
        ), f"receiver_elem in MethodCall {receiver_elem.__class__.__name__} is not an Expr"
        ...
        for a in args_data:
            ...
            assert isinstance(
                arg_elem, Expr
            ), f"arg_elem in MethodCall {arg_elem.__class__.__name__} is not an Expr"
        ...
        for v in value:
            ...
            assert isinstance(
                arg_elem, Expr
            ), f"arg_elem in AddArgs {arg_elem.__class__.__name__} is not an Expr"
        ...
        assert isinstance(
            condition, Expr
        ), f"condition in ConditionalStatement {condition.__class__.__name__} is not an Expr"
        ...
        if else_stmt_data:
            ...
            assert isinstance(
                else_elem, ConditionalStatement
            ), f"else_elem in ConditionalStatement {else_elem.__class__.__name__} is not an ConditionalStatement"
        ...
        assert isinstance(
            name, Expr
        ), f"name in AtomicUnit {name.__class__.__name__} is not an Expr"
        ...
        for attr in data.get("attributes", []):
            ...
            assert isinstance(
                attr_elem, Attribute
            ), f"attr_elem in AtomicUnit {attr_elem.__class__.__name__} is not an Attribute"
        ...
        for attr_data in data.get("attributes", []):
            ...
            assert isinstance(
                attr_elem, Attribute
            ), f"attr_elem in UnitBlock {attr_elem.__class__.__name__} is not an Attribute"
            unit.add_attribute(attr_elem)
        ...
        for comment_data in data.get("comments", []):
            ...
            ), f"comment_elem in UnitBlock {comment_elem.__class__.__name__} is not an Comment"
        ...
        for var_data in data.get("variables", []):
            ...
            assert isinstance(
                var_elem, Variable
            ), f"var_elem in UnitBlock {var_elem.__class__.__name__} is not an Variable"
        ...
        for au_data in data.get("atomic_units", []):
            ...
            assert isinstance(
                au_elem, AtomicUnit
            ), f"au_data in UnitBlock {au_elem.__class__.__name__} is not an AtomicUnit"
        ...
        for ub_data in data.get("unit_blocks", []):
            ...
            assert isinstance(
                ub_elem, UnitBlock
            ), f"ub_elem in UnitBlock {ub_elem.__class__.__name__} is not an UnitBlock"
        ...
        for dep_data in data.get("dependencies", []):
            ...
            assert isinstance(
                dep_elem, Dependency
            ), f"dep_data in UnitBlock {dep_elem.__class__.__name__} is not an Dependency"
