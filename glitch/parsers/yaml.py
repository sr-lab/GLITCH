...
class YamlParser(p.Parser, ABC):
    ...
    def __parse_jinja_node(self, node: jinja2.nodes.Node, base_info: ElementInfo) -> Expr:
        def split_keep(s: str, delimiter: str) -> List[str]:
        ...
        info.code = self.__get_content(
            ElementInfo(node.lineno, node.col + 1, node.end_lineno, node.end_col + 1, ""),
            split_keep(base_info.code, "\n"),
        )
