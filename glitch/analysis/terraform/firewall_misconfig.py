...
from glitch.repr.inter import AtomicUnit, Attribute, CodeElement, KeyValue, String, Boolean, UnitBlock
...
class TerraformFirewallMisconfig(TerraformSmellChecker):
    def _find_block_recursive(self, element: AtomicUnit | UnitBlock, name: str) -> bool:
        blocks = element.statements if isinstance(element, AtomicUnit) else element.statements + element.unit_blocks
        for stmt in blocks:
