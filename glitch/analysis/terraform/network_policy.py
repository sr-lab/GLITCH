    ...
    def check(self, element: CodeElement, file: str) -> List[Error]:
        ...
        if isinstance(element, AtomicUnit):
            if element.type == "resource.azurerm_network_security_rule":
                ...
                if isinstance(access, KeyValue) and isinstance(access.value, str) and access.value.lower() == "allow":
                    protocol = self.check_required_attribute(
                        element.attributes, [""], "protocol"
                    )
                    if isinstance(protocol, KeyValue) and isinstance(protocol.value, str) and protocol.value.lower() == "udp":
                        errors.append(
                            Error(
                                "sec_network_security_rules", access, file, repr(access)
                            )
                        )
                    elif isinstance(protocol, KeyValue) and isinstance(protocol.value, str) and protocol.value.lower() == "tcp":
                        dest_port_range = self.check_required_attribute(
                            element.attributes, [""], "destination_port_range"
                        )
                        port = isinstance(dest_port_range, KeyValue) and isinstance(dest_port_range.value, str) and dest_port_range.value.lower() in [
                            "22",
                            "3389",
                            "*",
                        ]
                        port_ranges, _ = self.iterate_required_attributes(
                            ...
                            lambda x: (isinstance(x.value, str) and x.value.lower() in ["22", "3389", "*"]),
                        )
                        ...
                        if port or port_ranges:
                            ...
                            if isinstance(source_address_prefix, KeyValue) and isinstance(source_address_prefix.value, str) and (
                ...
                if isinstance(access, KeyValue) and isinstance(access.value, str) and access.value.lower() == "allow":
                    ...
                    if isinstance(protocol, KeyValue) and isinstance(protocol.value, str) and protocol.value.lower() == "udp":
                        ...
                        if isinstance(dest_port_range, KeyValue) and isinstance(dest_port_range.value, str) and dest_port_range.value.lower() in [
                            "22",
                            "3389",
                            "*",
                        ]:
