...
class TerraformKeyManagement(TerraformSmellChecker):
    def _check_attribute(self, attribute: Attribute, atomic_unit: AtomicUnit, parent_name: str, file: str) -> List[Error]:
        ...
        if (attribute.name == "rotation_period" and atomic_unit.type == "resource.google_kms_crypto_key"):
            expr1 = r'\d+\.\d{0,9}s'
            expr2 = r'\d+s'
            if (re.search(expr1, attribute.value) or re.search(expr2, attribute.value)):
                ...
                return [Error('sec_key_management', attribute, file, repr(attribute))]
        elif (attribute.name == "kms_master_key_id" and ((atomic_unit.type == "resource.aws_sqs_queue"
            and attribute.value == "alias/aws/sqs") or  (atomic_unit.type == "resource.aws_sns_queue"
