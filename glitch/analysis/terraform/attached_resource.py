...
from glitch.repr.inter import AtomicUnit, CodeElement, KeyValue, Attribute, String, UnitBlock, Array
...
class TerraformAttachedResource(TerraformSmellChecker):
    def check(self, element: CodeElement, file: str) -> List[Error]:
        ...
        if isinstance(element, AtomicUnit):
            ...
            def check_value_for_resource(value_code: str, resource_types: List[str]) -> bool:
                value_lower = value_code.lower()
                for resource_type in resource_types:
                    if value_lower.startswith("${" + f"{resource_type}.") or value_lower.startswith(f"{resource_type}."):
            ...
            def check_attached_resource(
                ...
                for stmt in statements:
                    if isinstance(stmt, UnitBlock):
                        if check_attached_resource(stmt.attributes, stmt.statements, resource_types):
                            return True
                return False
            ...
            if element.type == "aws_route53_record":
                ...
                if type_A and not check_attached_resource(
                    element.attributes, element.statements, SecurityVisitor.POSSIBLE_ATTACHED_RESOURCES
                ):
                    errors.append(
